...
1/2/Q/scroll switch weapon · **E interact** (open the door, get in or out of a car) · Esc pause.
...
* **The front door and the district.** Levels 1–3 are the building. At level 4 the lobby's double
  glass doors unlock (`Gameplay/World/FrontDoor`, rule in the engine-free `FrontDoorState`); press E
...
Vent.Gameplay ─► Player, Weapons, Enemies, Vehicles, Core (GameManager, LevelDirector, FrontDoor, Atmosphere, VehicleDriver)
...
8. `Gameplay/World/FrontDoorState.cs` → `FrontDoor.cs` → `Player/Interaction/PlayerInteractor.cs`
...
* EditMode (`Assets/Tests/EditMode`): progression, level rules, difficulty monotonicity, event
  channels, pooling, audio synthesis, save file, the front door rule, the layer table and collision
  matrix (incl. Vehicle), the district's surfaces/names/spots/barrier/headroom, and the car prefabs and numbers.
* PlayMode (`Assets/Tests/PlayMode`): loads the generated Building scene and checks vents are on the
  ...
  contained, wake the outdoor vents and lift the fog; `VehicleTests` drive, get in and out, drive-by
